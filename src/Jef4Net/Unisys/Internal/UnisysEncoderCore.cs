...
internal static class UnisysEncoderCore
{
    internal static void Convert(Configuration config, EncoderFallback fallback, ref EncoderState state, ReadOnlySpan<char> input, Span<byte> output, bool flush, bool count, out int used, out int written, out bool completed)
    {
        ...
        while (true)
        {
            ...
            {
                if (!count) output[written] = (byte)state.Output;
                written = checked(written + 1); state.Output >>= 8; state.OutputCount--;
            }
            ...
            if (source.IsEmpty && state.High == 0) break;

            int scalar, consumed = 0; bool malformed = false; char high = state.High;
            if (high != 0)
            {
                if (source.IsEmpty && !flush) break;
                if (!source.IsEmpty && char.IsLowSurrogate(source[0])) { scalar = char.ConvertToUtf32(high, source[0]); consumed = 1; }
                else { scalar = high; malformed = true; }
            }
            else
            {
                ...
                {
                    if (source.Length > 1 && char.IsLowSurrogate(source[1])) { scalar = char.ConvertToUtf32(ch, source[1]); consumed = 2; }
                    else if (source.Length == 1 && !flush && !replacing) { state.High = ch; used++; break; }
                    else malformed = true;
                }
                else if (char.IsLowSurrogate(ch)) malformed = true;
            }
            state.High = '\0'; int index = high != 0 ? used - 1 : used;
            if (replacing) state.ReplacementIndex += consumed; else used += consumed;
            int sbcs = !malformed && config.Mixed ? Mapping.EncodeSbcs(scalar) : -1;
            int dbcs = !malformed ? Mapping.EncodeDbcs(scalar) : -1;
            if (sbcs >= 0) Emit(config, ref state, false, sbcs);
            else if (dbcs >= 0) Emit(config, ref state, true, dbcs);
            else Replace(fallback, ref state, scalar, index, replacing);
        }
    ...
    {
        if (config.Mixed && state.Dbcs != dbcs)
        {
            state.Push(0x93); state.Push(dbcs ? (byte)0x70 : (byte)0xF1); state.Dbcs = dbcs;
        }
        if (dbcs) state.Push((byte)(code >> 8));
    ...
    {
        if (recursive) throw new ArgumentException("Encoder fallback contains an unencodable character.");
        EncoderFallbackBuffer buffer = fallback.CreateFallbackBuffer();
        if (scalar > 0xFFFF) { int n = scalar - 0x10000; buffer.Fallback((char)(0xD800 + (n >> 10)), (char)(0xDC00 + (n & 1023)), index); }
        else buffer.Fallback((char)scalar, index);
