...
/// <summary>Provides stateful decoding for a Unisys encoding.</summary>
internal sealed class UnisysDecoder : Decoder
{
    private readonly Configuration config;
    private DecoderState state;

    /// <summary>Initializes a new instance of the <see cref="UnisysDecoder"/> class.</summary>
    /// <param name="encoding">The owning Unisys encoding.</param>
    internal UnisysDecoder(UnisysEncoding encoding)
    {
    ...
    {
        var copy = this.state;
        UnisysDecoderCore.Convert(this.config, this.Fallback!, ref copy, bytes, default, flush, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetChars(ReadOnlySpan<byte> bytes, Span<char> chars, bool flush)
    {
        ...
        UnisysDecoderCore.Convert(this.config, this.Fallback!, ref this.state, bytes, chars, flush, false, out _, out int count, out _);
        return count;
    }
    ...
    public override void Convert(ReadOnlySpan<byte> bytes, Span<char> chars, bool flush, out int bytesUsed, out int charsUsed, out bool completed)
        => UnisysDecoderCore.Convert(this.config, this.Fallback!, ref this.state, bytes, chars, flush, false, out bytesUsed, out charsUsed, out completed);
}
