...
/// <summary>A Unisys encoding. Obtain instances from <see cref="UnisysEncodingProvider"/>.</summary>
public sealed class UnisysEncoding : Encoding
{
    private readonly string name;
    private readonly Configuration configuration;

    /// <summary>Initializes a new instance of the <see cref="UnisysEncoding"/> class.</summary>
    /// <param name="name">The canonical name of the encoding.</param>
    /// <param name="config">The conversion configuration.</param>
    internal UnisysEncoding(string name, Configuration config)
    ...
    /// <inheritdoc/>
    public override Encoder GetEncoder() => new UnisysEncoder(this);

    /// <inheritdoc/>
    public override Decoder GetDecoder() => new UnisysDecoder(this);
    ...
    public override int GetByteCount(ReadOnlySpan<char> chars)
    {
        ...
        UnisysEncoderCore.Convert(this.configuration, this.EncoderFallback, ref state, chars, default, true, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetBytes(ReadOnlySpan<char> chars, Span<byte> bytes)
    {
        ...
        var state = new EncoderState { Dbcs = !this.configuration.Mixed };
        UnisysEncoderCore.Convert(this.configuration, this.EncoderFallback, ref state, chars, bytes, true, false, out _, out int written, out _);
        return written;
    }
    ...
    public override int GetCharCount(ReadOnlySpan<byte> bytes)
    {
        ...
        UnisysDecoderCore.Convert(this.configuration, this.DecoderFallback, ref state, bytes, default, true, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetChars(ReadOnlySpan<byte> bytes, Span<char> chars)
    {
        ...
        var state = new DecoderState { Dbcs = !this.configuration.Mixed };
        UnisysDecoderCore.Convert(this.configuration, this.DecoderFallback, ref state, bytes, chars, true, false, out _, out int written, out _);
        return written;
    }
