...
/// <summary>Provides stateful decoding for a Jbis encoding.</summary>
internal sealed class JbisDecoder : Decoder
{
    private readonly Configuration config;
    private DecoderState state;

    /// <summary>Initializes a new instance of the <see cref="JbisDecoder"/> class.</summary>
    /// <param name="encoding">The owning Jbis encoding.</param>
    internal JbisDecoder(JbisEncoding encoding)
    {
    ...
    {
        this.state = new DecoderState { Dbcs = !this.config.Mixed };
        base.Reset();
    }
    ...
    {
        var copy = this.state;
        JbisDecoderCore.Convert(this.config, this.Fallback!, ref copy, bytes, default, flush, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetChars(ReadOnlySpan<byte> bytes, Span<char> chars, bool flush)
    {
        ...
        JbisDecoderCore.Convert(this.config, this.Fallback!, ref this.state, bytes, chars, flush, false, out _, out int count, out _);
        return count;
    }
    ...
    public override void Convert(ReadOnlySpan<byte> bytes, Span<char> chars, bool flush, out int bytesUsed, out int charsUsed, out bool completed)
        => JbisDecoderCore.Convert(this.config, this.Fallback!, ref this.state, bytes, chars, flush, false, out bytesUsed, out charsUsed, out completed);
}
