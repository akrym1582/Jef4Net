...
/// <summary>A Nec encoding. Obtain instances from <see cref="NecEncodingProvider"/>.</summary>
public sealed class NecEncoding : Encoding
{
    private readonly string name;
    private readonly Configuration configuration;

    /// <summary>Initializes a new instance of the <see cref="NecEncoding"/> class.</summary>
    /// <param name="name">The canonical name of the encoding.</param>
    /// <param name="config">The conversion configuration.</param>
    internal NecEncoding(string name, Configuration config)
        : base(0, new EncoderReplacementFallback(config.InitialJips || config.Mixed ? "\u3000" : "?"), new DecoderReplacementFallback("\uFFFD"))
    {
        this.name = name;
    ...
    /// <inheritdoc/>
    public override bool IsSingleByte => !this.configuration.Mixed && !this.configuration.InitialJips;

    /// <inheritdoc/>
    public override byte[] GetPreamble() => Array.Empty<byte>();

    /// <inheritdoc/>
    public override Encoder GetEncoder() => new NecEncoder(this);

    /// <inheritdoc/>
    public override Decoder GetDecoder() => new NecDecoder(this);
    ...
    {
        var state = new EncoderState { Jips = this.configuration.InitialJips };
        NecEncoderCore.Convert(this.configuration, this.EncoderFallback, ref state, chars, default, true, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetBytes(ReadOnlySpan<char> chars, Span<byte> bytes)
    {
        ...
        var state = new EncoderState { Jips = this.configuration.InitialJips };
        NecEncoderCore.Convert(this.configuration, this.EncoderFallback, ref state, chars, bytes, true, false, out _, out int written, out _);
        return written;
    }
    ...
    {
        var state = new DecoderState { Jips = this.configuration.InitialJips };
        NecDecoderCore.Convert(this.configuration, this.DecoderFallback, ref state, bytes, default, true, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetChars(ReadOnlySpan<byte> bytes, Span<char> chars)
    {
        ...
        var state = new DecoderState { Jips = this.configuration.InitialJips };
        NecDecoderCore.Convert(this.configuration, this.DecoderFallback, ref state, bytes, chars, true, false, out _, out int written, out _);
        return written;
    }
