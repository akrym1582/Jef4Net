...
/// <summary>Provides stateful encoding for a Nec encoding.</summary>
internal sealed class NecEncoder : Encoder
{
    private readonly Configuration config;
    private EncoderState state;

    /// <summary>Initializes a new instance of the <see cref="NecEncoder"/> class.</summary>
    /// <param name="encoding">The owning Nec encoding.</param>
    internal NecEncoder(NecEncoding encoding)
    {
    ...
    {
        this.state = new EncoderState { Jips = this.config.InitialJips };
        base.Reset();
    }
    ...
    {
        var copy = this.state;
        NecEncoderCore.Convert(this.config, this.Fallback!, ref copy, chars, default, flush, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetBytes(ReadOnlySpan<char> chars, Span<byte> bytes, bool flush)
    {
        ...
        NecEncoderCore.Convert(this.config, this.Fallback!, ref this.state, chars, bytes, flush, false, out _, out int count, out _);
        return count;
    }
    ...
    public override void Convert(ReadOnlySpan<char> chars, Span<byte> bytes, bool flush, out int charsUsed, out int bytesUsed, out bool completed)
        => NecEncoderCore.Convert(this.config, this.Fallback!, ref this.state, chars, bytes, flush, false, out charsUsed, out bytesUsed, out completed);
}
