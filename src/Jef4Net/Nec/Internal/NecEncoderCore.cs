...
internal static class NecEncoderCore
{
    /// <summary>Converts UTF-16 input to Nec bytes.</summary>
        ...
        out bool completed)
    {
        ...
        while (true)
        {
            ...
            {
                if (flush && config.Mixed && state.Jips != config.InitialJips)
                {
                    state.Push(config.IsE ? (byte)0x3F : (byte)0x1A);
                    state.Push(config.InitialJips ? (config.IsE ? (byte)0x75 : (byte)0x70) : (config.IsE ? (byte)0x76 : (byte)0x71));
            ...
            if (state.Prefix != 0)
            {
                ...
                int pair = scalar > 0 && !malformed ? Mapping.Encode(true, config.IsE, Mapping.Key(prefix, scalar), config.HanyoDenshi) : -1;
                if (pair >= 0)
                {
            ...
            if (!malformed && (config.Mixed || config.InitialJips) && Mapping.IsPrefix(scalar, config.HanyoDenshi))
            {
    ...
    {
        if (config.Mixed || !config.InitialJips)
        {
            int code = Mapping.Encode(false, config.IsE, key, config.HanyoDenshi);

            // Mixed encodings reserve the two-byte shift prefix.
            if (code >= 0 && (!config.Mixed || code != (config.IsE ? 0x3F : 0x1A)))
            {
        ...
        if (config.Mixed || config.InitialJips)
        {
            int code = Mapping.Encode(true, config.IsE, key, config.HanyoDenshi);
            if (code >= 0)
            {
    ...
    private static void Emit(Configuration config, ref EncoderState state, bool jips, int code)
    {
        if (config.Mixed && state.Jips != jips)
        {
            state.Push(config.IsE ? (byte)0x3F : (byte)0x1A);
            state.Push(jips ? (config.IsE ? (byte)0x75 : (byte)0x70) : (config.IsE ? (byte)0x76 : (byte)0x71));
            state.Jips = jips;
        }
