...
/// <summary>Provides stateful decoding for a Nec encoding.</summary>
internal sealed class NecDecoder : Decoder
{
    private readonly Configuration config;
    private DecoderState state;

    /// <summary>Initializes a new instance of the <see cref="NecDecoder"/> class.</summary>
    /// <param name="encoding">The owning Nec encoding.</param>
    internal NecDecoder(NecEncoding encoding)
    {
    ...
    {
        this.state = new DecoderState { Jips = this.config.InitialJips };
        base.Reset();
    }
    ...
    {
        var copy = this.state;
        NecDecoderCore.Convert(this.config, this.Fallback!, ref copy, bytes, default, flush, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetChars(ReadOnlySpan<byte> bytes, Span<char> chars, bool flush)
    {
        ...
        NecDecoderCore.Convert(this.config, this.Fallback!, ref this.state, bytes, chars, flush, false, out _, out int count, out _);
        return count;
    }
    ...
    public override void Convert(ReadOnlySpan<byte> bytes, Span<char> chars, bool flush, out int bytesUsed, out int charsUsed, out bool completed)
        => NecDecoderCore.Convert(this.config, this.Fallback!, ref this.state, bytes, chars, flush, false, out bytesUsed, out charsUsed, out completed);
}
