...
/// <summary>Provides stateful decoding for a Fujitsu encoding.</summary>
internal sealed class FujitsuDecoder : Decoder
{
    private readonly Configuration config;
    private DecoderState state;

    /// <summary>Initializes a new instance of the <see cref="FujitsuDecoder"/> class.</summary>
    ...
    {
        this.state = new DecoderState { Jef = this.config.InitialJef };
        base.Reset();
    }
    ...
    {
        var copy = this.state;
        FujitsuDecoderCore.Convert(this.config, this.Fallback!, ref copy, bytes, default, flush, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetChars(ReadOnlySpan<byte> bytes, Span<char> chars, bool flush)
    {
        ...
        FujitsuDecoderCore.Convert(this.config, this.Fallback!, ref this.state, bytes, chars, flush, false, out _, out int count, out _);
        return count;
    }
    ...
    public override void Convert(ReadOnlySpan<byte> bytes, Span<char> chars, bool flush, out int bytesUsed, out int charsUsed, out bool completed)
        => FujitsuDecoderCore.Convert(this.config, this.Fallback!, ref this.state, bytes, chars, flush, false, out bytesUsed, out charsUsed, out completed);
}
