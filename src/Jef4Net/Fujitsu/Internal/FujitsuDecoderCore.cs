...
{
    /// <summary>Converts Fujitsu bytes to UTF-16 output.</summary>
        ...
        out bool completed)
    {
        ...
        while (true)
        {
            ...
            {
                state.K2 = false;
                if (used < input.Length && input[used] == 0xE2)
                {
                    used++;
                    state.Jef = true;
                    continue;
                ...
                Replace(fallback, ref state, new byte[] { 0x30 }, used - 1);
                continue;
            }
