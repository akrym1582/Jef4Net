...
internal static class Mapping
{
    ...
    internal static ulong Decode(bool jef, int kind, int code)
    {
        if (jef && code >> 8 is >= 0x80 and <= 0xA0 && (code & 255) is >= 0xA1 and <= 0xFE)
            return Key(0xE000 + ((code >> 8) - 0x80) * 94 + (code & 255) - 0xA1);
        if (jef) return FujitsuTables.JefEntries[FujitsuTables.JefIds[code]];
        return (kind == 0 ? FujitsuTables.LowerDecode : kind == 1 ? FujitsuTables.KanaDecode : FujitsuTables.AsciiDecode)[code];
    }
    internal static int Encode(bool jef, int kind, ulong key)
    {
        int scalar = Scalar(key);
        if (jef && Suffix(key) == 0 && scalar is >= 0xE000 and <= 0xEC1D)
            return ((0x80 + (scalar - 0xE000) / 94) << 8) | (0xA1 + (scalar - 0xE000) % 94);
        var keys = jef ? FujitsuTables.JefKeys : kind == 0 ? FujitsuTables.LowerKeys : kind == 1 ? FujitsuTables.KanaKeys : FujitsuTables.AsciiKeys;
        var codes = jef ? FujitsuTables.JefCodes : kind == 0 ? FujitsuTables.LowerCodes : kind == 1 ? FujitsuTables.KanaCodes : FujitsuTables.AsciiCodes;
    ...
    internal static bool IsPrefix(int scalar) => FujitsuTables.JefPrefixes.IndexOf(scalar) >= 0;
}
...
internal readonly struct Configuration
{
    ...
    internal Configuration(bool mixed, bool jef, int kind) { Mixed = mixed; InitialJef = jef; Kind = kind; }
}
