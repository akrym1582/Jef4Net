...
{
    internal static void Convert(Configuration config, EncoderFallback fallback, ref EncoderState state,
        ReadOnlySpan<char> input, Span<byte> output, bool flush, bool count,
        out int used, out int written, out bool completed)
    {
        ...
        while (true)
        {
            ...
            if (source.IsEmpty && state.High == 0 && state.Prefix == 0)
            {
                ...
                { state.Push(config.InitialJef ? (byte)0x28 : (byte)0x29); state.Jef = config.InitialJef; continue; }
                break;
            }
            ...
            if (high != 0)
            {
                ...
                { scalar = char.ConvertToUtf32(high, source[0]); consumed = 1; }
            ...
            {
                scalar = source[0]; consumed = 1;
                if (char.IsHighSurrogate(source[0]))
                {
                    if (source.Length > 1 && char.IsLowSurrogate(source[1]))
                    { scalar = char.ConvertToUtf32(source[0], source[1]); consumed = 2; }
                    else if (source.Length == 1 && !flush && !replacing)
                    { state.High = source[0]; used++; break; }
                    else malformed = true;
                }
                else if (char.IsLowSurrogate(source[0])) malformed = true;
            }
            if (state.Prefix != 0)
            {
    ...
    {
        if (config.Mixed && state.Jef != jef) { state.Push(jef ? (byte)0x28 : (byte)0x29); state.Jef = jef; }
        if (jef) state.Push((byte)(code >> 8));
