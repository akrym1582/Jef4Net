...
internal static class IbmEncoderCore
{
    /// <summary>Converts UTF-16 input to Ibm bytes.</summary>
        ...
        out bool completed)
    {
        ...
        while (true)
        {
            ...
            {
                if (flush && config.Mixed && state.Dbcs != config.InitialDbcs)
                {
                    state.Push(config.InitialDbcs ? (byte)0x0E : (byte)0x0F);
            ...
            if (state.Prefix != 0)
            {
                ...
                int pair = scalar > 0 && !malformed ? Mapping.Encode(true, config.DbcsKind, Mapping.Key(prefix, scalar), false) : -1;
                if (pair >= 0)
                {
            ...
            if (!malformed && (config.Mixed || config.InitialDbcs) && Mapping.IsPrefix(scalar, false))
            {
    ...
    {
        if (config.Mixed || !config.InitialDbcs)
        {
            int code = Mapping.Encode(false, config.SbcsKind, key, false);

            // Mixed encodings reserve these bytes for shift syntax (including K2 prefix).
            if (code >= 0 && (!config.Mixed || code is not 0x0E and not 0x0F))
            {
        ...
        if (config.Mixed || config.InitialDbcs)
        {
            int code = Mapping.Encode(true, config.DbcsKind, key, false);
            if (code >= 0)
            {
    ...
    private static void Emit(Configuration config, ref EncoderState state, bool dbcs, int code)
    {
        ...
        {
            state.Push(dbcs ? (byte)0x0E : (byte)0x0F);
            state.Dbcs = dbcs;
        }
