...
/// <summary>Provides stateful encoding for a Ibm encoding.</summary>
internal sealed class IbmEncoder : Encoder
{
    private readonly Configuration config;
    private EncoderState state;

    /// <summary>Initializes a new instance of the <see cref="IbmEncoder"/> class.</summary>
    /// <param name="encoding">The owning Ibm encoding.</param>
    internal IbmEncoder(IbmEncoding encoding)
    {
    ...
    {
        this.state = new EncoderState { Dbcs = this.config.InitialDbcs };
        base.Reset();
    }
    ...
    {
        var copy = this.state;
        IbmEncoderCore.Convert(this.config, this.Fallback!, ref copy, chars, default, flush, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetBytes(ReadOnlySpan<char> chars, Span<byte> bytes, bool flush)
    {
        ...
        IbmEncoderCore.Convert(this.config, this.Fallback!, ref this.state, chars, bytes, flush, false, out _, out int count, out _);
        return count;
    }
    ...
    public override void Convert(ReadOnlySpan<char> chars, Span<byte> bytes, bool flush, out int charsUsed, out int bytesUsed, out bool completed)
        => IbmEncoderCore.Convert(this.config, this.Fallback!, ref this.state, chars, bytes, flush, false, out charsUsed, out bytesUsed, out completed);
}
