...
/// <summary>An IBM encoding. Obtain instances from <see cref="IbmEncodingProvider"/>.</summary>
public sealed class IbmEncoding : Encoding
{
    private readonly string name;
    private readonly Configuration configuration;

    /// <summary>Initializes a new instance of the <see cref="IbmEncoding"/> class.</summary>
    /// <param name="name">The canonical name of the encoding.</param>
    /// <param name="config">The conversion configuration.</param>
    internal IbmEncoding(string name, Configuration config)
        : base(0, new EncoderReplacementFallback(config.InitialDbcs || config.Mixed ? "\u3000" : "?"), new DecoderReplacementFallback("\uFFFD"))
    {
        this.name = name;
    ...
    /// <inheritdoc/>
    public override bool IsSingleByte => !this.configuration.Mixed && !this.configuration.InitialDbcs;

    /// <inheritdoc/>
    public override byte[] GetPreamble() => Array.Empty<byte>();

    /// <inheritdoc/>
    public override Encoder GetEncoder() => new IbmEncoder(this);

    /// <inheritdoc/>
    public override Decoder GetDecoder() => new IbmDecoder(this);
    ...
    {
        var state = new EncoderState { Dbcs = this.configuration.InitialDbcs };
        IbmEncoderCore.Convert(this.configuration, this.EncoderFallback, ref state, chars, default, true, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetBytes(ReadOnlySpan<char> chars, Span<byte> bytes)
    {
        ...
        var state = new EncoderState { Dbcs = this.configuration.InitialDbcs };
        IbmEncoderCore.Convert(this.configuration, this.EncoderFallback, ref state, chars, bytes, true, false, out _, out int written, out _);
        return written;
    }
    ...
    {
        var state = new DecoderState { Dbcs = this.configuration.InitialDbcs };
        IbmDecoderCore.Convert(this.configuration, this.DecoderFallback, ref state, bytes, default, true, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetChars(ReadOnlySpan<byte> bytes, Span<char> chars)
    {
        ...
        var state = new DecoderState { Dbcs = this.configuration.InitialDbcs };
        IbmDecoderCore.Convert(this.configuration, this.DecoderFallback, ref state, bytes, chars, true, false, out _, out int written, out _);
        return written;
    }
