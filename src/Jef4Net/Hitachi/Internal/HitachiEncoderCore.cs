...
internal static class HitachiEncoderCore
{
    /// <summary>Converts UTF-16 input to Hitachi bytes.</summary>
        ...
        out bool completed)
    {
        ...
        while (true)
        {
            ...
            {
                if (flush && config.Mixed && state.Keis != config.InitialKeis)
                {
                    state.Push(0x0A);
                    state.Push(config.InitialKeis ? (byte)0x42 : (byte)0x41);
            ...
            if (state.Prefix != 0)
            {
                ...
                int pair = scalar > 0 && !malformed ? Mapping.Encode(true, config.KeisKind, Mapping.Key(prefix, scalar), config.HanyoDenshi) : -1;
                if (pair >= 0)
                {
            ...
            if (!malformed && (config.Mixed || config.InitialKeis) && Mapping.IsPrefix(scalar, config.HanyoDenshi))
            {
    ...
    {
        if (config.Mixed || !config.InitialKeis)
        {
            int code = Mapping.Encode(false, config.SbcsKind, key, config.HanyoDenshi);

            // Mixed encodings reserve these bytes for shift syntax (including K2 prefix).
            if (code >= 0 && (!config.Mixed || code != 0x0A))
            {
        ...
        if (config.Mixed || config.InitialKeis)
        {
            int code = Mapping.Encode(true, config.KeisKind, key, config.HanyoDenshi);
            if (code >= 0)
            {
    ...
    private static void Emit(Configuration config, ref EncoderState state, bool keis, int code)
    {
        if (config.Mixed && state.Keis != keis)
        {
            state.Push(0x0A);
            state.Push(keis ? (byte)0x42 : (byte)0x41);
            state.Keis = keis;
        }
