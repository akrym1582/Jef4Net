...
/// <summary>Provides stateful encoding for a Hitachi encoding.</summary>
internal sealed class HitachiEncoder : Encoder
{
    private readonly Configuration config;
    private EncoderState state;

    /// <summary>Initializes a new instance of the <see cref="HitachiEncoder"/> class.</summary>
    /// <param name="encoding">The owning Hitachi encoding.</param>
    internal HitachiEncoder(HitachiEncoding encoding)
    {
    ...
    {
        this.state = new EncoderState { Keis = this.config.InitialKeis };
        base.Reset();
    }
    ...
    {
        var copy = this.state;
        HitachiEncoderCore.Convert(this.config, this.Fallback!, ref copy, chars, default, flush, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetBytes(ReadOnlySpan<char> chars, Span<byte> bytes, bool flush)
    {
        ...
        HitachiEncoderCore.Convert(this.config, this.Fallback!, ref this.state, chars, bytes, flush, false, out _, out int count, out _);
        return count;
    }
    ...
    public override void Convert(ReadOnlySpan<char> chars, Span<byte> bytes, bool flush, out int charsUsed, out int bytesUsed, out bool completed)
        => HitachiEncoderCore.Convert(this.config, this.Fallback!, ref this.state, chars, bytes, flush, false, out charsUsed, out bytesUsed, out completed);
}
