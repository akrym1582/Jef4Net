...
/// <summary>A Hitachi encoding. Obtain instances from <see cref="HitachiEncodingProvider"/>.</summary>
public sealed class HitachiEncoding : Encoding
{
    private readonly string name;
    private readonly Configuration configuration;

    /// <summary>Initializes a new instance of the <see cref="HitachiEncoding"/> class.</summary>
    /// <param name="name">The canonical name of the encoding.</param>
    /// <param name="config">The conversion configuration.</param>
    internal HitachiEncoding(string name, Configuration config)
        : base(0, new EncoderReplacementFallback(config.InitialKeis || config.Mixed ? "\u3000" : "?"), new DecoderReplacementFallback("\uFFFD"))
    {
        this.name = name;
    ...
    /// <inheritdoc/>
    public override bool IsSingleByte => !this.configuration.Mixed && !this.configuration.InitialKeis;

    /// <inheritdoc/>
    public override byte[] GetPreamble() => Array.Empty<byte>();

    /// <inheritdoc/>
    public override Encoder GetEncoder() => new HitachiEncoder(this);

    /// <inheritdoc/>
    public override Decoder GetDecoder() => new HitachiDecoder(this);
    ...
    {
        var state = new EncoderState { Keis = this.configuration.InitialKeis };
        HitachiEncoderCore.Convert(this.configuration, this.EncoderFallback, ref state, chars, default, true, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetBytes(ReadOnlySpan<char> chars, Span<byte> bytes)
    {
        ...
        var state = new EncoderState { Keis = this.configuration.InitialKeis };
        HitachiEncoderCore.Convert(this.configuration, this.EncoderFallback, ref state, chars, bytes, true, false, out _, out int written, out _);
        return written;
    }
    ...
    {
        var state = new DecoderState { Keis = this.configuration.InitialKeis };
        HitachiDecoderCore.Convert(this.configuration, this.DecoderFallback, ref state, bytes, default, true, true, out _, out int count, out _);
        return count;
    }
    ...
    public override int GetChars(ReadOnlySpan<byte> bytes, Span<char> chars)
    {
        ...
        var state = new DecoderState { Keis = this.configuration.InitialKeis };
        HitachiDecoderCore.Convert(this.configuration, this.DecoderFallback, ref state, bytes, chars, true, false, out _, out int written, out _);
        return written;
    }
