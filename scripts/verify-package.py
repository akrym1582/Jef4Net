...
def verify_repository_copy(archive, package_path):
    """Require a packaged document to be byte-identical to its repository copy."""
    repository_path = repository_root.joinpath(*PurePosixPath(package_path).parts)
    if not repository_path.is_file():
        sys.exit(f"Missing repository license file: {package_path}")
    if archive.read(package_path) != repository_path.read_bytes():
        sys.exit(f"Packaged license file differs from repository file: {package_path}")
...
with ZipFile(packages[0]) as archive:
    ...
    for required in ("LICENSE", "THIRD-PARTY-NOTICES.md"):
        ...
        verify_repository_copy(archive, required)
    ...
    notice = (repository_root / "THIRD-PARTY-NOTICES.md").read_text(encoding="utf-8")
    ...
    for ref in refs:
        verify_repository_copy(archive, ref)
